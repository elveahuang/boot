...
import cc.elvea.boot.commons.utils.GenericsUtils;
...
import static cc.elvea.boot.commons.constants.GlobalConstants.DEFAULT_BATCH_SIZE;

/**
 * @author elvea
 * @since 24.1.0
 */
public interface EntityService<T extends IdEntity, K extends Serializable, R extends BaseRepository<T, K>>
        extends Service {
    ...
    default Class<R> currentRepositoryClass() {
    ...
    default boolean hasId(final Object object) {
        if (!ObjectUtils.isEmpty(object) && object instanceof IdEntity entity) {
            return !ObjectUtils.isEmpty(entity.getId()) && entity.getId() > 0;
        }
        return false;
    }
