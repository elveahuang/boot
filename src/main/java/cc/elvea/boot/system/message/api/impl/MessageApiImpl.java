package cc.elvea.boot.system.message.api.impl;

import cc.elvea.boot.commons.constants.GlobalConstants;
import cc.elvea.boot.commons.enums.*;
import cc.elvea.boot.commons.utils.*;
...
import cc.elvea.boot.system.message.model.dto.MessageUserDto;
import cc.elvea.boot.system.message.model.dto.SendMessageDto;
import cc.elvea.boot.system.message.model.entity.*;
import cc.elvea.boot.system.message.sender.MessageMailSender;
import cc.elvea.boot.system.message.sender.MessageNoticeSender;
import cc.elvea.boot.system.message.sender.MessageSender;
import cc.elvea.boot.system.message.service.*;
import cc.elvea.boot.system.template.HtmlTemplateService;
...
import static cc.elvea.boot.system.commons.constants.SystemMessageConstants.TPL_CLASSPATH;
...
public class MessageApiImpl implements MessageApi {
    ...
    public Long createMessage(CreateMessageDto message) throws Exception {
        ...
        MessageEntity entity = MessageEntity.builder()
                ...
                .data(CollectionUtils.isEmpty(message.getParams()) ? "" : JacksonUtils.toJson(message.getParams()))
...
//            this.messageService.send(SendMessageAmqpDto.builder().id(messageId).build());
        }
        return messageId;
    }
...
//                    this.messageAmqpService.send(SendMessageAmqpDto.builder().id(messageEntity.getId()).build());
    ...
    public void sendMessage(Long messageId, boolean force) throws Exception {
        ...
        MessageTypeEntity messageTypeEntity = this.messageTypeService.findById(messageTypeId);
        if (messageTypeEntity == null) {
    ...
    private MessageUserEntity createMessageUser(Long messageId, MessageUserDto userDto) {
        ...
        if (!ObjectUtils.isEmpty(userDto.getUserId()) && userDto.getUserId() > 0) {
            userEntity = this.userService.findById(userDto.getUserId());
        }
